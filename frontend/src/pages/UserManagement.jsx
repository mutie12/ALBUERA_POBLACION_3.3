...
function UserManagement() {
  ...
  const getRoleBadge = (role) => {
    const colors = {
      admin: { bg: "#dbeafe", text: "#1e40af", iconName: "shield" },
      respondent: { bg: "#d1fae5", text: "#065f46", iconName: "ambulance" },
      resident: { bg: "#f3f4f6", text: "#374151", iconName: "user" }
    };
    ...
    return (
      <span style={{ 
        ...
        <Icon name={style.iconName} size={14} /> {role.charAt(0).toUpperCase() + role.slice(1)}
      </span>
    );
  };
  ...
  const getStatusBadge = (status) => {
    const styles = {
      pending: { bg: "#fef3c7", text: "#92400e", iconName: "clock" },
      approved: { bg: "#d1fae5", text: "#065f46", iconName: "check" },
      rejected: { bg: "#fee2e2", text: "#991b1b", iconName: "x" }
    };
    ...
    return (
      <span style={{ 
        ...
        <Icon name={style.iconName} size={14} /> {status.charAt(0).toUpperCase() + status.slice(1)}
      </span>
    );
  };
  ...
  return (
    <div className="admin-page-container">
      ...
      <div className="page-header">
        <h1><Icon name="users" size={28} /> User Management</h1>
        <div style={{ display: "flex", gap: "12px" }}>
          <button 
            onClick={() => setShowVerifyModal(true)}
            className="btn btn-outline"
          >
            <Icon name="key" size={16} /> Verify with Code
          </button>
          <select 
            value={filter} 
        ...
        <div style={{ marginBottom: "32px" }}>
          <div style={{ 
            ...
            <h2 style={{ fontSize: "1.125rem", marginBottom: "12px", color: "#92400e" }}>
              <Icon name="clock" size={20} /> Pending Approvals ({pendingResidents.length})
            </h2>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    ...
                    <div>
                      ...
                      <div style={{ display: "flex", gap: "8px", marginTop: "8px" }}>
                        <span style={{ fontSize: "0.8125rem", color: "var(--text-secondary)" }}>
                          <Icon name="location" size={14} /> {user.barangay}
                        </span>
                        {user.phone && (
                          <span style={{ fontSize: "0.8125rem", color: "var(--text-secondary)" }}>
                  ...
                  <div style={{ display: "flex", gap: "8px" }}>
                    {user.email ? (
                      ...
                      >
                        <Icon name="mail" size={14} /> Send Code
                      </button>
                    ) : (
                      <span style={{ 
                        fontSize: "0.75rem", 
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead style={{ background: "var(--gray-50)" }}>
            <tr>
              ...
              <th style={{ padding: "16px 20px", textAlign: "left", fontWeight: "600", color: "var(--text-secondary)", fontSize: "0.8125rem" }}>Station / Dept</th>
            ...
            {users.length === 0 ? (
              ...
              users.map((user) => (
                <tr key={user._id} style={{ borderTop: "1px solid var(--gray-100)" }}>
                  ...
                  <td style={{ padding: "16px 20px", fontSize: "0.875rem", color: "var(--text-secondary)" }}>
                    {user.role === "respondent" ? user.department || user.station || "-" : "-"}
                  </td>
                  <td style={{ padding: "16px 20px" }}>
        ...
        <div className="modal-overlay" onClick={() => setShowVerifyModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: "400px" }}>
            <div className="modal-header" style={{ background: "var(--primary-600)", color: "white" }}>
              <h2><Icon name="key" size={22} /> Verify Resident with Code</h2>
              <button 
