...
function Dashboard(props) {
  ...
  const fetchReports = async () => {
    try {
      ...
      setError(err.response?.data?.message || "Failed to load reports. Please check your connection.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      case "Pending": return <Icon name="clock" size={18} />;
      case "Responding": return <Icon name="siren" size={18} />;
      case "Resolved": return <Icon name="checkCircle" size={18} />;
      case "Declined": return <Icon name="xCircle" size={18} />;
      default: return "";
    }
  };
  ...
  const getEmergencyIcon = (type) => {
    switch (type) {
      case "Fire": return <Icon name="flame" size={20} />;
      case "Flood": return <Icon name="water" size={20} />;
      case "Medical Emergency": return <Icon name="heart" size={20} />;
      case "Vehicular Accident": return <Icon name="car" size={20} />;
      case "Crime/Violence": return <Icon name="shield" size={20} />;
      case "Natural Disaster": return <Icon name="cloud" size={20} />;
      case "Earthquake": return <Icon name="activity" size={20} />;
      case "Typhoon": return <Icon name="cloud" size={20} />;
      case "Landslide": return <Icon name="activity" size={20} />;
      default: return <Icon name="alert" size={20} />;
    }
  };
    ...
    <div className="admin-page-container">
      ...
      {role === "admin" && (
        <div className="dashboard-welcome admin-welcome">
          <h2><Icon name="shield" size={28} /> Welcome back, {name || "Admin"}!</h2>
      ...
      {(showDashboardView && role === "respondent") && (
        <div className="dashboard-welcome respondent-welcome">
          <h2><Icon name="ambulance" size={28} /> Welcome back, {name || "Respondent"}!</h2>
      ...
      {(showDashboardView && role === "resident") && (
        <div style={{ background: 'linear-gradient(135deg, #0ea5e9 0%, #06b6d4 100%)', padding: '48px 40px', borderRadius: '24px', marginBottom: '32px', width: '100%' }}>
          <h2 style={{ fontSize: '2rem', fontWeight: '700', color: 'white', marginBottom: '8px' }}><Icon name="home" size={32} /> Welcome to Your Community, {name || "Resident"}!</h2>
      ...
      <div className="stats-grid-modern">
        <div className="stat-card-modern">
          <div className="stat-icon total" style={{ display: "flex", alignItems: "center", justifyContent: "center" }}><Icon name="barChart" size={32} color="white" /></div>
        ...
        <div className="stat-card-modern">
          <div className="stat-icon pending" style={{ display: "flex", alignItems: "center", justifyContent: "center" }}><Icon name="clock" size={32} color="white" /></div>
          <div className="stat-info">
            <h3>{pendingCount}</h3>
            <p>Pending</p>
          </div>
        ...
        <div className="stat-card-modern">
          <div className="stat-icon responding" style={{ display: "flex", alignItems: "center", justifyContent: "center" }}><Icon name="siren" size={32} color="white" /></div>
          <div className="stat-info">
        ...
        <div className="stat-card-modern">
          <div className="stat-icon resolved" style={{ display: "flex", alignItems: "center", justifyContent: "center" }}><Icon name="checkCircle" size={32} color="white" /></div>
          <div className="stat-info">
            <h3>{resolvedCount}</h3>
            <p>Resolved</p>
          </div>
        ...
        {role === "admin" && (
          <div className="stat-card-modern">
            <div className="stat-icon respondent" style={{ display: "flex", alignItems: "center", justifyContent: "center" }}><Icon name="users" size={32} color="white" /></div>
            <div className="stat-info">
      ...
      {role === "admin" && (
        <div style={{ marginBottom: "32px" }}>
          <div className="section-header">
            <h2><Icon name="file" size={24} /> Published News <span className="count-badge">{news.length}</span></h2>
          </div>
          {news.length === 0 ? (
              ...
              {news.map((item) => (
                ...
                >
                  <span className={`news-category ${item.category}`}>
                    {item.category === "announcement" ? <Icon name="bellRing" size={14} /> : 
                     item.category === "safety-tip" ? <Icon name="alertCircle" size={14} /> : 
                     item.category === "alert" ? <Icon name="siren" size={14} /> : <Icon name="file" size={14} />}
      ...
      {(showDashboardView && role === "respondent") && (
        <div className="empty-state-enhanced">
          <div className="empty-icon"><Icon name="ambulance" size={48} color="#059669" /></div>
          ...
          <div style={{ display: 'flex', gap: '12px', marginTop: '8px', flexWrap: 'wrap', justifyContent: 'center' }}>
            <Link to="/respondent" className="btn btn-primary" style={{ background: '#059669' }} state={{ view: "queue" }}>
              <Icon name="siren" size={16} /> Emergency Queue ({pendingCount} pending)
            </Link>
            <Link to="/respondent" className="btn btn-warning" state={{ view: "responses" }}>
              <Icon name="alertCircle" size={16} color="#f59e0b" /> My Active Responses ({respondingCount})
            </Link>
          </div>
          ...
          {pendingCount > 0 && (
            <div className="alert-banner warning" style={{ maxWidth: '500px', marginTop: '20px' }}>
              <div className="alert-icon"><Icon name="alert" size={24} /></div>
      ...
      {(showDashboardView && role === "resident") && (
        <div className="empty-state-enhanced">
          <div className="empty-icon"><Icon name="home" size={48} /></div>
      ...
      {(role === "resident" || role === "respondent") && filteredNews.length > 0 && (
        <div style={{ marginBottom: "32px" }}>
          <div className="section-header">
            <h2><Icon name="file" size={24} /> News & Updates <span className="count-badge">{filteredNews.length}</span></h2>
          </div>
      ...
      {role === "resident" && (
        <div style={{ marginBottom: "32px" }}>
          <div className="section-header">
            <h2><Icon name="siren" size={22} /> Community Emergency Reports <span className="count-badge">{filteredReports.length}</span></h2>
          </div>
          {filteredReports.length === 0 ? (
            <div className="empty-state-enhanced">
              <div className="empty-icon"><Icon name="shield" size={48} /></div>
                  ...
                  <div className="card-header">
                    <div className="emergency-badge">
                      ...
                      <div className="type-info">
                        ...
                        <span><Icon name="location" size={14} /> {report.location}</span>
                      </div>
                    </div>
        ...
        <>
            ...
            <div className="section-header" style={{ marginBottom: "24px" }}>
              <h2>
                {role === "respondent" && location.state?.view === "responses"
                  ? <><Icon name="alertCircle" size={22} color="#ef4444" /> My Active Response</>
                  : role === "respondent"
                    ? <><Icon name="siren" size={22} /> Emergency Queue</>
                    : <><Icon name="siren" size={22} /> Emergency Reports</>}
          ...
          {displayReports.length === 0 ? (
            <div className="empty-state">
              <span className="icon"><Icon name="shield" size={36} /></span>
              <h3>
                {role === "respondent" && location.state?.view === "responses" 
                  ? "No Active Responses" 
                  : role === "respondent" 
                    ...
                    <div className="emergency-info">
                      ...
                      <div>
                        ...
                        <span className="emergency-location">
                          <Icon name="location" size={14} /> {report.location}
                        </span>
                      </div>
                    </div>
                  ...
                  {(role === "admin" || role === "respondent") && report.status !== "Resolved" && (
                    <div className="report-card-footer action-buttons">
                      {role === "respondent" && report.status === "Pending" && (
                        ...
                        >
                          <Icon name="siren" size={16} /> Accept & Respond
                        </button>
                      )}
                      {role === "respondent" && report.status === "Pending" && (
                        <button
                        ...
                        >
                          <Icon name="xCircle" size={16} color="white" /> Decline
                        </button>
                      )}
                      {role === "admin" && report.status === "Pending" && (
                        <button
                          ...
                          >
                            <Icon name="folder" size={16} /> Assign Respondent
                          </button>
                        )}
                        <button
                        ...
                        >
                          <Icon name="checkCircle" size={16} color="white" /> Mark Resolved
                      </button>
                    </div>
                  )}
                </div>
        ...
        <div className="modal-overlay" onClick={() => setShowAssignModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: "450px" }}>
            <div className="modal-header">
              <h2><Icon name="folder" size={22} /> Assign Respondent</h2>
              <button 
            ...
            <div className="modal-footer">
              ...
              <button 
                ...
                style={{ background: "#0d9488" }}
              >
                {assigning ? "Assigning..." : <><Icon name="send" size={16} /> Send Assignment</>}
              </button>
            </div>
          </div>
        </div>
      ...
      {showMessageModal && (
        <div className="modal-overlay" onClick={() => setShowMessageModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: "500px" }}>
            <div className="modal-header">
              <h2>{isBroadcast ? <><Icon name="bellRing" size={22} /> Broadcast Message</> : <><Icon name="message" size={22} /> Send Message</>}</h2>
              <button 
            ...
            <div className="modal-footer">
              ...
              >
                {sendingMessage ? "Sending..." : (isBroadcast ? <><Icon name="bellRing" size={16} /> Broadcast</> : <><Icon name="message" size={16} /> Send Message</>)}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="modal-overlay" onClick={handleCloseNewsModal}>
          <div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: "550px" }}>
            <div className="modal-header" style={{ background: editingNewsId ? "#f59e0b" : "#8b5cf6", color: "white" }}>
              <h2>{editingNewsId ? <><Icon name="edit" size={22} /> Update News</> : <><Icon name="file" size={22} /> Create News & Updates</>}</h2>
              <button 
            ...
            <div className="modal-body">
              ...
              <div className="form-group">
                ...
                >
                  <option value="announcement"><Icon name="bellRing" size={16} /> Announcement</option>
                  <option value="safety-tip"><Icon name="alertCircle" size={16} /> Safety Tip</option>
                  <option value="update"><Icon name="refresh" size={16} /> Community Update</option>
                  <option value="alert"><Icon name="siren" size={16} /> Alert</option>
                </select>
              </div>
            ...
            <div className="modal-footer">
              ...
              >
                {creatingNews ? "Saving..." : editingNewsId ? <><Icon name="edit" size={16} /> Update News</> : <><Icon name="send" size={16} /> Publish News</>}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="modal-overlay" onClick={() => setShowEmergencyModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: "550px" }}>
            <div className="modal-header" style={{ background: "#dc2626", color: "white" }}>
              <h2><Icon name="siren" size={22} /> Send Emergency Alert</h2>
              <button 
            ...
            <div className="modal-body">
              <div style={{ marginBottom: "16px", padding: "12px", background: "#fef2f2", borderRadius: "8px", border: "1px solid #fecaca" }}>
                <strong style={{ color: "#dc2626" }}><Icon name="alert" size={18} color="#dc2626" /> Warning:</strong> This will send an emergency alert to ALL residents and respondents via in-app notification, SMS, and email.
              </div>
              ...
              <div className="form-group">
                ...
                <div style={{ display: "flex", gap: "12px", marginTop: "4px" }}>
                  <label style={{ display: "flex", alignItems: "center", gap: "8px", cursor: "pointer" }}>
                    ...
                    />
                    <Icon name="phone" size={16} /> SMS Only
                  </label>
                  <label style={{ display: "flex", alignItems: "center", gap: "8px", cursor: "pointer" }}>
                    <input
                      type="radio"
                      ...
                      onChange={(e) => setEmergencyType(e.target.value)}
                    />
                    <Icon name="mail" size={16} /> Email Only
                  </label>
                  <label style={{ display: "flex", alignItems: "center", gap: "8px", cursor: "pointer" }}>
                    <input
                      type="radio"
            ...
            <div className="modal-footer">
              ...
              >
                {sendingEmergency ? "Sending..." : <><Icon name="siren" size={16} color="white" /> Send Emergency Alert</>}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="modal-overlay" onClick={() => setShowCreateRespondentModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: "500px" }}>
            <div className="modal-header" style={{ background: "#10b981", color: "white" }}>
              <h2><Icon name="plus" size={24} style={{ marginRight: "6px" }} /> Create Respondent Account</h2>
              <button 
            ...
            <div className="modal-body">
              ...
              <div className="form-group">
                ...
                <select
                  ...
                  style={{
                    width: "100%", padding: "10px", borderRadius: "8px", border: "1px solid #d1d5db", fontSize: "1rem"
                  }}
                >
              ...
              {createdAccount && (
                <div style={{ marginTop: "16px", padding: "16px", background: "#fef3c7", borderRadius: "8px", border: "1px solid #f59e0b" }}>
                  <strong style={{ color: "#92400e" }}><Icon name="checkCircle" size={18} style={{ marginRight: "4px" }} /> Account Created Successfully!</strong>
            ...
            <div className="modal-footer">
              ...
              >
                {creatingRespondent ? "Creating..." : <><Icon name="checkCircle" size={18} style={{ marginRight: "4px" }} /> Create Account</>}
              </button>
            </div>
          </div>
        </div>
