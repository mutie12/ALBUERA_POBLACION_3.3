...
function SendMessage() {
  ...
  const handleChange = (e) => {
    ...
    if (name === "recipientId") {
      if (value === "SELECT_ALL_RESIDENTS") {
        setFormData({ ...formData, recipientId: value, recipientName: "All Residents" });
        return;
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const isBroadcast = formData.recipientId === "SELECT_ALL_RESIDENTS";
      const body = {
        title: formData.title,
        message: fullMessage,
        isBroadcast,
        recipientIds: isBroadcast ? null : null
      };
      if (isBroadcast) {
        body.recipientIds = "ALL";
      } else {
        body.recipientId = formData.recipientId;
        body.recipientName = formData.recipientName;
      }
      ...
      setSuccess(
        isBroadcast
          ? <><Icon name="check" size={16} /> Message broadcast to {count} resident{count !== 1 ? "s" : ""}!</>
          : <><Icon name="check" size={16} /> Message sent successfully!</>
      );
    ...
    <div className="admin-page-container">
      <div style={{ background: "white", borderRadius: "12px", padding: "32px", boxShadow: "0 2px 8px rgba(0,0,0,0.1)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px" }}>
          <h2 style={{ margin: 0, color: "#1f2937" }}><Icon name="send" size={24} /> Send Message</h2>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ marginBottom: "20px" }}>
            ...
            >
              ...
              {/* Respondents grouped by station/department */}
              {(() => {
                const stations = [...new Set(
                  users.filter(u => u.role === "respondent")
                    .map(u => u.station || u.department || "Unspecified Station")
                )].sort();
                return stations.map(s => (
                  <optgroup key={s} label={`Station / Dept: ${s}`}>
                    {users
                      .filter(u => u.role === "respondent" && (u.station || u.department || "Unspecified Station") === s)
                      .map(user => (
                        <option key={user._id} value={user._id}>
                          {user.name} ({user.station || user.department || "N/A"})
                        </option>
                      ))}
                  </optgroup>
                ));
...
<option value="SELECT_ALL_RESIDENTS">
                   {formData.recipientId === "SELECT_ALL_RESIDENTS" ? <><Icon name="check" size={14} /> </> : ""}Select All Residents
                 </option>
          ...
          <div style={{ marginBottom: "20px" }}>
            <label style={{ display: "block", marginBottom: "8px", fontWeight: "500" }}>
              <Icon name="location" size={16} /> Location (Barangay) *
            </label>
            <select
              name="barangay"
          ...
          >
            {loading ? "Sending..." : <><Icon name="send" size={16} /> Send Message</>}
          </button>
        </form>
      </div>
    </div>
