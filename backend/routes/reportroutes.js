...
// Create a report (admin, respondent, or resident)
router.post("/", auth, async (req, res) => {
  try {
    if (!req.user || (req.user.role !== "admin" && req.user.role !== "respondent" && req.user.role !== "resident")) {
      return res.status(403).json({ message: "Not authorized to create reports" });
    }

    const { location, emergencyType, description, name, image } = req.body;
    ...
    const report = await Report.create({
      reporterName: req.user.name || name || "Anonymous",
...
// Update report resolution (admin/respondent only)
router.patch("/:id/resolution", auth, async (req, res) => {
  try {
    if (!req.user || (req.user.role !== "admin" && req.user.role !== "respondent")) {
      return res.status(403).json({ message: "Not authorized" });
    }
    ...
    console.error("Update report resolution error:", err);
    res.status(500).json({ message: "Server error updating report" });
  }
});
