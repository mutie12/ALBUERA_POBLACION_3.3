...
router.post("/register", async (req, res) => {
  try {
    const { name, username, password, barangay, barangayCode, email, phone, role, department, badgeNumber, vehicleNumber } = req.body;
...
router.post("/login", async (req, res) => {
  try {
    ...
    res.json({ token, role: user.role, name: user.name, userId: user._id, department: user.department || "" });
  } catch (err) {
...
router.put("/users/:id", auth, async (req, res) => {
  ...
  try {
    const { name, email, phone, barangay, role, department, station, badgeNumber, vehicleNumber } = req.body;
    
    // Non-admins can update name, email, phone, department, badgeNumber, vehicleNumber
    const updateData = isAdmin 
      ? { name, email, phone, barangay, role, department, station, badgeNumber, vehicleNumber }
      : { name, email, phone, department, badgeNumber, vehicleNumber };
